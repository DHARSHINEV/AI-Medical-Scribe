...
export default function PrescriptionPage() {
  ...
  const update = (medicationId: string, key: keyof MedicationItem, value: string) =>
    setMedications((current) =>
      current.map((m) => (m.id === medicationId ? { ...m, [key]: value } : m))
    )

  const genderDisplay = c.currentPatient.gender || c.currentPatient.sex || 'Unspecified'
  const allergies = c.currentPatient.allergies || []

  return (
    <>
      <Notice />
      <PageTitle eyebrow={`PRESCRIPTION · #${c.consultation.id}`} title="Prepare prescription">
        <span className="status-pill">
          <i /> Draft · clinician review required
        </span>
      </PageTitle>

      <div className="prescription-layout">
        <Panel title="Prescription" label="No medications are pre-populated">
          <div className="document-header">
            <b>MediScribe</b>
            <span>Clinical prescription draft · {new Date().toLocaleDateString()}</span>
          </div>

          <div className="patient-banner">
            <div>
              <small>Patient</small>
              <strong>{c.currentPatient.name}</strong>
              <span>
            ...
            <div className="allergy-banner">
              <b>Allergies</b>
              <span>{allergies.length > 0 ? allergies.join(', ') : 'NKDA'}</span>
            </div>
          </div>

          <label className="field-label">
            Assessment
            <input defaultValue={c.note.Assessment || c.note.assessment} />
          </label>
          ...
          <div className="signature-box">
            <ShieldCheck size={20} />
            <span>
              <b>Clinician signature required</b>
              <small>Draft has not been sent or signed.</small>
            </span>
          </div>
          ...
          <div className="review-actions">
            ...
            <Button
              onClick={() => {
                ...
                a.href = url
                a.download = `consultation-${c.consultation.id}-prescription.json`
                a.click()
